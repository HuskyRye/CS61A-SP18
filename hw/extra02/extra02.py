...
def add_fns(f_and_df, g_and_dg):
    ...
    def add(x, derive):
        return f_and_df(x, derive) + g_and_dg(x, derive)
    return add
...
def mul_fns(f_and_df, g_and_dg):
    ...
    def mul(x, derive):
        if not derive:
            ...
            return (f_and_df(x, True) * g_and_dg(x, False)
                    +f_and_df(x, False) * g_and_dg(x, True))
    return mul
...
def letters(tree):
    ...
    if is_huffman_leaf(tree):
        ...
        return sum([letters(branch) for branch in branches(tree)], [])
...
def decode_one(tree, code):
    ...
    [0, 1, 1, 1, 1]
    """
    while not is_huffman_leaf(tree):
        tree = branches(tree)[code[0]]
        code.pop(0)
    return letter(tree)
...
def encodings(tree):
    ...
    if is_huffman_leaf(tree):
        ...
        left_dict = encodings(branches(tree)[0])
        for symbol, encode in left_dict.items():
            left_dict[symbol] = [0] + encode           
        right_dict = encodings(branches(tree)[1])
        for symbol, encode in right_dict.items():
            right_dict[symbol] = [1] + encode            
...
def huffman(frequencies):
    ...
    while len(leaves) != 1:
        leaves.append(huffman_tree(leaves.pop(0), leaves.pop(0)))
        leaves.sort(key=lambda leaf: weight(leaf))
    return leaves[0]
