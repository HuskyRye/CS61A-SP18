...
def balanced(m):
    ...
    if not balanced(end(left_side)) or not balanced(end(right_side)):
        return False
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) < extreme and extreme < upper_bound(x):
...
def polynomial(x, c):
    ...
    guesses = [lower + k * step for k in range(num_steps)]
    # print(guesses)

    # Find extremums
    extremums = list()
    for guess in guesses:
        extreme = find_zero(df, ddf, guess)
        if extreme not in extremums:
            extremums.append(extreme)
    # print(extremums)

    # Find lower_bound and upper_bound
    values = [f(x) for x in extremums if lower < x and x < upper] + [f(lower), f(upper)]
    # print(values)
