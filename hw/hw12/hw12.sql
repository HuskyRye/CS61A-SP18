...
CREATE TABLE siblings AS
  SELECT d1.child AS name1, d2.child AS name2 
  FROM parents AS d1, parents AS d2
  WHERE d1.parent = d2.parent AND d1.child < d2.child;

-- Sentences about siblings that are the same size
CREATE TABLE sentences AS
  SELECT name1 || " and " || name2 || " are " || s1.size || " siblings"
  FROM siblings, size_of_dogs AS s1, size_of_dogs AS s2
  WHERE name1 = s1.name AND name2 = s2.name AND s1.size = s2.size;
...
CREATE TABLE stacks2 AS
	SELECT d1.name || ", " || d2.name || ", " || d3.name || ", " || d4.name,
			d1.height + d2.height + d3.height + d4.height as stack_height
	FROM dogs AS d1, dogs AS d2, dogs AS d3, dogs AS d4
	WHERE d1.height < d2.height AND d2.height < d3.height
		AND d3.height < d4.height
		AND d1.height + d2.height + d3.height + d4.height > 170
	ORDER BY stack_height;