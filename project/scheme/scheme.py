...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('incorrect number of arguments: {0}'.format(self.name))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_expressions = Pair(param, body)
        lambda_procedure = do_lambda_form(lambda_expressions, env)
        env.define(name, lambda_procedure)
        return name
        # END PROBLEM 10
    else:
